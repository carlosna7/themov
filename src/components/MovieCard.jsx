...
const MovieCard = ({movie, showLink = true}) => {
  return (
    <div className='flex flex-col rounded-lg justify-center w-[23%] gap-2 mb-8 bg-slate-950'>
        ...
        {showLink && <Link className='flex justify-center font-bold p-1 mb-3 mx-3 bg-sky-300 border-2 border-sky-300 rounded-lg text-black hover:bg-transparent hover:border-blue-400 hover:text-blue-400 duration-500' href={`/movie/${movie.id} `}>Detalhes</Link>}
    </div>
  )
}
